...
The swath coordinate is mandatory as it describes the instrument scanning and stepping in terms of the latitude and longitude for each footprint. This greatly reduces the complexity of the geolocation, which to generalize would require a two-body orbit model, precise time information, 3D instrument pointing information (e.g., cross-track, conical scan), etc. to determine the latitude/longitude of each footprint. 
...
* The MODIS Level 1b data (see <<MODIS: HDF-EOS Format,Example 21>> in Appendix A) stores latitude/longitude for every 5th line and element, beginning at position 3,3.
...
The latitude/longitude coordinate reference is geodetic (planetodetic for other planetary bodies). The slowest varying dimension of the geolocation coordinate must represent forward (along-track) movement of the platform. 

The following example is a minimum specification for the swath coordinate.

[caption="Example {counter:example-count}: "]
.Geolocation example for a scanning instrument: along track (atrack) and cross track (xtrack).
====
