...
group: ancillary {
  variables:
    float ancil_data(time, xtrack, band);
      :coordinates = "/time /geolocation/lat /geolocation/lon /band";
}

group: geolocation {
  variables:
    float lat_vertex(time, xtrack, vertex=4);

    float lon_vertex(time, xtrack, vertex=4);
